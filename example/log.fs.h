/****************************************************************************************************************************
	...
	с наименьшим порядковым номером (ID), после этого используя функцию LogFs_readFile () можно прочитать содержимое 
	...
	Внимание!
		...
		открытом файле вызов LogFs_createFile() вернет ошибку.
	...
	LogFs_format();                            // Готовим носитель для файловой системы форматированием
	if (LogFs_initialize() == FS_SUCCESS)      // Инициализируем файловую систему, и проверяем получилось ли
	{
		LogFs_createFile();                    // Создаём сессию
		LogFs_writeToCurrentFile(buffer, 5);   // Пишем из буфера в файл 5 байт	
		LogFs_writeToCurrentFile(buffer, 5);   // Еще раз пишем из буфера в файл 5 байт
		// Далее писать в файл можно сколько угодно до следующего вызова LogFs_initialize()
	}
	...
	{
		LogFs_createFile();               	     // Создаём новую сессию
		LogFs_writeToCurrentFile(buffer, 5);     // Пишем из буфера 4 раза в файл по 5 байт
		LogFs_writeToCurrentFile(buffer, 5);
		LogFs_writeToCurrentFile(buffer, 5);
		LogFs_writeToCurrentFile(buffer, 5);
		LogFs_initialize();                      // Закроем сессию
	}
	// Перейдем к чтению
	...
	// И прочитаем файл в буфер с 0 байта сразу целиком (size байт)
	...
	// Если файл с таким номером существуствует, функция откроет его на чтение и вернет FS_SUCCESS, в противном случае FS_ERROR
	if (LogFs_findFileByNum(num) == FS_SUCCESS)
	{
		// Читаем файл за раз
